...
4. In Association, the relationship between classes is that a class is shown have another sub-class inside it. On the other hand, the advanced relationship I implemented includes a class inheriting properties from another class while also having an object class that has a HAS-A relationship. In other words, the difference between these is that the relationship is now directly related to each other and not just associated with one other.
